...
**Status: DONE — ported, deployed, cut over 2026-06-13** (same day as this
eval). See `_journal/2026-06-13.md` § "Python→Mix port DONE" for what actually
happened. Two things changed vs the plan below: (1) the three gaps were already
filled — both boxes were on **mix 0.17.1**, not the 0.17.0 assumed here, so
`stat()`/`chown()`/`read_file_bytes(max)` are all native; (2) a **4th gap**
surfaced and was the real one — `bytes_to_string` is strict-UTF8, so the 8 KB
header sniff rejected the many real messages with raw 8-bit bytes before the
ASCII header. Fixed in **mix 0.17.2**: `bytes_to_string($b, {lossy:true})`. The
port is tracked at `deploy/spamlite-stats-collect`. The eval below is kept as
the historical record.

**Status (original): evaluation only — no code written yet.** This doc is the
journaled test case for the general situation "a production python3 script needs
to become Mix" (the never-Python rule). Every claim below was verified against
mix 0.17.0 (`/opt/cosmix/bin/mix`, the binary now on both the dev box and mrn) or
...
rationale lives in the private ops journal (2026-04-11).
