...
- Ham source is `Maildir/cur` (the INBOX). Mail filed into other folders is
  ...
  INBOX delivery time.
- Spam source is `.Junk/cur`. Users who flag normal mail as spam (ivy) poison
...
| user | outcome | detail |
|------|---------|--------|
| gina@agency-a.example.net.au | **wrote** | balanced err 0.1276 → 0.1226, max_interesting 150→200; corpus is 40:1 ham:spam and fp-heavy — a data-hygiene case the params can only nudge |
| evan@corp-c.example.com | refused | no generalising gain |
| ivy@corp-c.example.com | refused | known data-hygiene case (mislabeled spam folder) |

felix@corp-d.example.com.au, run manually the same day against his **live** corpus
