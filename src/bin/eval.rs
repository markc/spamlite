...
fn usage_and_exit() -> ! {
    eprintln!(
        "spamlite-eval — offline accuracy harness\n\
         ...
         --combine-mode M       fisher|geometric (fisher)\n  \
         --new-word-score P     fallback prob for unknown/thin tokens (0.5)\n  \
         --min-distance D       drop tokens within D of 0.5 (0.0 = off)\n  \
         --min-array-size N     min tokens kept under --min-distance (0)\n  \
...
fn parse_args() -> Args {
    ...
    while i < argv.len() {
        ...
        match a {
            ...
            "--new-word-score" => {
                params.new_word_score = need(i, a).parse().expect("f64");
                i += 2;
            }
            "--min-distance" => {
                params.min_distance = need(i, a).parse().expect("f64");
                i += 2;
            }
            "--min-array-size" => {
                params.min_array_size = need(i, a).parse().expect("usize");
                i += 2;
            }
            "--threshold" | "-t" => {
